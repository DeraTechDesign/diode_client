...
var (
	defaultMode      = "rw"
	domainPattern    = regexp.MustCompile(`^(.+)\.(derateknoloji|diode|derateknoloji\.com|diode\.link|diode\.ws)(:[\d]+)?$`)
...
func parseHost(host string) (isWS bool, mode string, deviceID string, port int, err error) {
	...
	if domain == "diode.link" || domain == "diode" || domain == "diode.ws" || domain == "derateknoloji.com" {
